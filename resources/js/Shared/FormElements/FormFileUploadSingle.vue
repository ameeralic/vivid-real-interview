        ...
        >
            ...
            <div class="flex gap-2">
              <div class="flex text-white hover:text-black gap-1 items-center py-1 px-4 bg-blue-600 hover:bg-white rounded-full shadow-lg" @click="$refs.fileInputSingle.click()">
                    ...
                    <p
                        v-text="
                            addedFile || uploadedFile() ? 'Edit' : 'Add'
                        "
                        class="text-sm"
                    ></p>
                </div>
                <div class="flex text-gray-50 hover:text-red-500 gap-1 items-center" @click="fileRemoved()" v-if="addedFile || uploadedFile()">
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        fill="none"
                        viewBox="0 0 24 24"
            ...
            <div
                ...
                :class="{ 'rounded-full': rounded, 'rounded-lg': !rounded, 'saturate-50 bg-blend-overlay bg-gray-900/75 shadow-2xl' : (addedFile || uploadedFile()), 'border-2 border-rose-500': error || errorSize,}"
    ...
    methods: {
        fileAdded(files) {
            ...
            if (!this.errorSize) {
                ...
                this.$emit("fileChange", this.addedFile ? this.addedFile : [null]);
            }
        },
        fileRemoved() {
            this.errorSize = false;
        ...
        uploadedFile() {
            if (this.addedFile) {
                ...
                if((this.oldImage.split("/").pop()).split('.').length > 1){
